...
-- KABINA (Lan Kabinak)
CREATE TABLE KABINA (
    kabina_id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE MAKINA (
    makina_id INT AUTO_INCREMENT PRIMARY KEY,
    izena VARCHAR(100) NOT NULL,
    mota VARCHAR(100),
...
CREATE TABLE GORAGAILUA (
    goragailua_id INT AUTO_INCREMENT PRIMARY KEY,
    izena VARCHAR(100) NOT NULL,
    kabina_id INT NOT NULL UNIQUE,
    FOREIGN KEY (kabina_id) REFERENCES KABINA(kabina_id) ON DELETE CASCADE
);
